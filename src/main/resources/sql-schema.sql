...
create table if not exists ims.orderline (order_id int, item_id int, foreign key (order_id) REFERENCES ims.orders (order_id), FOREIGN KEY (item_id) REFERENCES ims.items (item_id));